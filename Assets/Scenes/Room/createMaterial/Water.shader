Shader "Unlit/Water"
{
    Properties
    {
      ...
      _Amp ("Amplititude",Range(0,1)) = 0.5
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            Interpolator vert (MeshData v)
            {
                ...
                float wave = sin(v.vertex.x + _Time.y);
                float wave2 = sin(v.vertex.z + _Time.y);
                float wave3 = sin(v.vertex.y + _Time.y);
                // float wave3 = cos((v.uv.z - _Time.y * 0.1) *TAU*5 );
                
                v.vertex.y += (wave+wave2 + wave3) * _Amp;
